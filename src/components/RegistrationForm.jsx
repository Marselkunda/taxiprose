...
const RegistrationForm = () => {
  ...
  const isOnlyNumbers = (value) => /^[0-9]*$/.test(value); // For tel, postalNumber
  const isCompanyNameValid = (value) => /^[a-zA-Z0-9\s'’"!@#%&(),.-]*$/.test(value); // Allows text, numbers, special chars
  const isOrgNumberValid = (value) => /^[0-9-]*$/.test(value); // Allows numbers + "-"
  ...
  const handleChange = (e) => {
    ...
    if (["postalNumber", "tel"].includes(name) && !isOnlyNumbers(value))
      return;

    if (name === "companyName" && !isCompanyNameValid(value)) return;

    if (name === "organizationNumber" && !isOrgNumberValid(value)) return;
